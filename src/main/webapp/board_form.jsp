		...
		<div class="wrap ac">
			<div id="container_wrap">
				...
				<form id="board-field" action="${actionUrl}" method="post">
					...
					<table id = "board_table" width="400" border="1" cellspacing="0" cellpadding="0"  
						align="center">
						<tr>
							<td align="center" colspan="2"><c:set var="TitleName"
									value="CREATE_JSP_BOARD" /> <c:if test="${isView}">
						...
						<tr>
							...
							<td width="330"><input type="hidden" name="num"
								value="${board.num}" /><%=request.getParameter("index")%></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td width="330"><input type="hidden" name="email"
						...
						<tr>
							<td colspan="2" align="center"><c:choose>
									...
									<c:when test="${isView}">
										...
										<c:if test="${isUser==true||isMaster==true}">
											<!-- master아이디는 모든 게시물 수정삭제 가능 -->
											<input type="button" name="delete" value="Delete"
												onclick="location.href='/board/removeBoard?num=${board.num}'" />
									...
									<c:otherwise>
										<input
											style="margin-top: 3px; background-color: #337ab7; color: #fff;"
											type="submit" value="Submit" />
										<input style="margin-top: 3px; background-color: #337ab7;"
											bnt-custom" type="reset" value="Reset" />
									</c:otherwise>
								</c:choose> <input
								style="margin-top: 3px; background-color: #337ab7;class="
								bnt-custom" type="button" value="List"
								onclick="location.href='/board/Boardlist'" /></td>
						</tr>
					</table>
						...
						<div class="wrap2">
							<form action="/replyReply" method="post">
								<input type="hidden" name="num" value="${board.num}" /> <input
									type="text" size="80" maxlength="200" name="reContent" value="">
							...
							<%
								boolean yn = true;
										if (request.getSession().getAttribute("isMaster") == null)
											yn = false;
										if (request.getSession().getAttribute("userId").equals(reBoard.getUserId()) || yn == true) {
							%>
							<!-- 마스터 아이디는 모든댓글 삭제 가능 -->
							<div class="x">
								<input class="x" type="button" name="X" value="X"
									onclick="location.href='/board/removeReBoard?reNum=<%=reBoard.getNum()%>&bNum=<%=reBoard.getBoardNum()%>'" />
							</div>
							<%
								}
							%>
