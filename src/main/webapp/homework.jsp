...
<body>
				...
				<div id="container">
					<div id="top">
						...
						<form name="frm" action="/homeworks/homeworkList" method="get">
							과목 선택 : <SELECT name="subjectNameList" onChange="change();">
								<OPTION style="color: black" value="-1" name="none">조회할
									과목을 선택하세요.</OPTION>
								<%
									for (int i = 0; i < allSubjectCount; i++) {
								%>
								<OPTION style="color: black" value="<%=allSubjectName[i]%>"><%=allSubjectName[i]%></OPTION>
								<%
									}
								%>
							</SELECT> <input class="btn btn-custom" type="submit" value="조회">
						</form>
						...
						<div id="homework-header">
							<table id="board_table" border="1px" cellpadding="0"
								...
								<tr height="50">
									...
									<td><a
										href="/homeworks/viewHomework?num=<%=homework.getHomeworkNum()%>&index=<%=i + 1%>&homework_userId=<%=homework.getUserId()%>">
 ...
 %>
					...
					<div id="homework-footer">
						<c:if test="${isMaster }">
							<button id="homework-button"
								onclick="location.href='/homeworks/createHomeworkForm?subjectName=${subjectName}'">과제생성</button>
						</c:if>
					</div>
					<%
						}
...
>>>>>>> branch 'master' of https://github.com/gnudbproject/dbproject.git
			</div>
		</div>
<<<<<<< HEAD
=======
